...
export class LoginPage {
      ...
      <div style="position:fixed; inset:0; display:flex; align-items:center; justify-content:center; background:var(--color-bg); padding:var(--space-6); z-index:999;">
        <div style="max-width:380px; width:100%; text-align:center;">
          ...
          <button id="guest-mode-btn"
            style="width:100%; padding:12px 24px; border:none; background:none; color:var(--color-text-muted); font-size:14px; cursor:pointer; margin-top:var(--space-3);">
            로그인 없이 사용하기
          </button>
          <p style="font-size:12px; color:var(--color-text-muted); margin-top:var(--space-2);">로그인하면 모든 기기에서 데이터가 동기화됩니다</p>
        </div>
      </div>
    `;
  }
  ...
  async mount() {
    ...
    document.getElementById('guest-mode-btn')?.addEventListener('click', () => {
      ...
      window.dispatchEvent(new CustomEvent('guest-mode-enter'));
    });
  }
